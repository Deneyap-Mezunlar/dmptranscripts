...
if(!process.env.HIDE_TRANSCRIPT_WARNINGS && isDJSv14)
    console.log('[WARN] DMPTranscripts v13 için tasarlandı, lakin v14 kullanıyorsunuz. Bu sorunlara sebep olabilir.');
...
async function generateTranscript<T extends ReturnTypes>(
    ...
    document.getElementById('tickettopic')!.textContent = ` #${channel.name} adlı destek talebinin başlangıcı.`;
    if (channel.topic && channel.topic != null) {
        document.getElementById('tickettopic')!.innerHTML = `#${he.escape(channel.name)} adlı destek talebinin başlangıcı. ${formatContent(channel.topic, channel, false, true)}`;
    }
